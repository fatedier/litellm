...
_NOVA_COST_DISCOUNT_FIELD = "nova_cost_discount"
...
def cost_per_token(
    ...
    if call_type == "speech" or call_type == "aspeech":
        ...
        if custom_llm_provider == "vertex_ai" and _transcription_usage_has_token_details(usage_block):
...
def _apply_cost_discount(
    ...
    """
    Apply deployment-specific Nova discount or provider-specific discount config.
    ...
    nova_cost_discount = _get_nova_cost_discount(model=model, model_info=model_info)
    discount_source = custom_llm_provider
    if nova_cost_discount is not None:
        discount_percent = nova_cost_discount
        discount_source = _NOVA_COST_DISCOUNT_FIELD
...
def _get_nova_cost_discount(
    model: str | None = None,
    model_info: ModelInfo | None = None,
) -> float | None:
    discount: Any = None
    if model_info is not None:
        discount = cast(dict, model_info).get(_NOVA_COST_DISCOUNT_FIELD)
    ...
    if discount is None and model is not None:
        ...
        if isinstance(registered_model_info, dict):
            discount = registered_model_info.get(_NOVA_COST_DISCOUNT_FIELD)
