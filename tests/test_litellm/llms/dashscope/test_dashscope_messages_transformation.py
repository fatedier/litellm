...
def test_should_route_dashscope_provider_to_native_messages_config():
    glm_config = ProviderConfigManager.get_provider_anthropic_messages_config(
        model="glm-5.1",
        provider=LlmProviders.DASHSCOPE,
    )
    qwen_config = ProviderConfigManager.get_provider_anthropic_messages_config(
        model="qwen-plus",
        provider=LlmProviders.DASHSCOPE,
    )

    assert isinstance(glm_config, DashScopeAnthropicMessagesConfig)
    assert isinstance(qwen_config, DashScopeAnthropicMessagesConfig)
